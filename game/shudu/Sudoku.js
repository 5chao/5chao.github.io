...
Sudoku = function(size) {
	this.size = size;
	this.version = 'v1.0';
    this.gameBox = this.get('game-box');
};
...
Sudoku.prototype = {
	...
	initLayout : function() {
		...
		this.gameBox.appendChild(westPanel);
		...
		closeBtn.style.cssText = 'position:absolute;left:100px;top:0;width:20px;height:18px;'
				+ 'cursor:pointer;cursor:hander;text-align:right;color:red;background:url(../../images/close.gif) no-repeat;';
		...
		for (var i = 0; i < this.size; i++) {
			...
			numi.innerHTML = i + 1;
			numi.style.cssText = "position:absolute;text-align:center;font-size:28px;left:"
					+ (i % 3)
					* 40
					+ "px;top:"
		...
		for (var i = 0; i < this.size; i++) {
			for (var j = 0; j < this.size; j++) {
                ...
                cell.className = 'cell_item';

				cell.style.cssText = "left:" + j * w
						+ "px;top:" + i * h + "px;width:" + w + "px;height:"
						+ h + "px;line-height:"+ h +"px;";
			...
			for (var j = 0; j < this.size; j++) {
				...
				if (rdm > checkedIndex) {
					...
					cell.onclick = function(e) {
						...
						np.style.left = pos.x +'px';
						np.style.top = pos.y + 'px';
